...
class C2SPathContextDataModule(LightningDataModule):
    def __init__(self, config: DictConfig, vocabulary: Vocabulary_c2s):
        ...
        self._dataset_dir = join(config.data_folder, config.name,
                                 config.dataset.name)
        self._train_data_file = join(self._dataset_dir, "train.c2s")
        self._val_data_file = join(self._dataset_dir, "val.c2s")
        self._test_data_file = join(self._dataset_dir, "test.c2s")
