...
from models.sysevr.data_classes import SYSSample


class SYSDataset(IterableDataset):
    ...
    def __next__(self) -> Tuple[numpy.ndarray, int, int]:
        if self._cur_sample_idx >= self._end_sample_idx:
            raise StopIteration()
        gadget, label, metric_info, words_for_label = self._cur_buffered_path_context[
        ...
        return SYSSample(tokens=gadget, label=label, metric_info=metric_info, n_tokens=words_for_label)
