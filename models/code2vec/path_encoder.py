...
from models.code2seq.data_classes import FROM_TOKEN, TO_TOKEN, PATH_NODES
...
class PathEncoder(nn.Module):
    def __init__(
        ...
        self.token_embedding = nn.Embedding(n_tokens,
                                            config.embedding_size,
                                            padding_idx=token_pad_id)
        self.node_embedding = nn.Embedding(n_nodes,
                                           config.embedding_size,
        ...
        concat_size = config.embedding_size * 3
        self.embedding_dropout = nn.Dropout(config.embedding_dropout)
        self.linear = nn.Linear(concat_size, out_size, bias=False)
        self.norm = nn.LayerNorm(out_size)

    @staticmethod
    def _calculate_concat_size(embedding_size: int, rnn_size: int,
                               num_directions: int) -> int:
    ...
    def _path_nodes_embedding(self, path_nodes: torch.Tensor) -> torch.Tensor:
        ...
        with torch.no_grad():
            is_contain_pad_id, first_pad_pos = torch.max(
                path_nodes == self.node_pad_id, dim=0)
            first_pad_pos[~is_contain_pad_id] = path_nodes.shape[
        ...
        packed_path_nodes = nn.utils.rnn.pack_padded_sequence(
            path_nodes_embeddings, sorted_path_lengths)
        ...
        encoded_paths = h_t[-self.num_directions:].transpose(0, 1).reshape(
            h_t.shape[1], -1)
    ...
    def _concat_with_linear(
            self, encoded_contexts: List[torch.Tensor]) -> torch.Tensor:
    ...
    def forward(self, contexts: Dict[str, torch.Tensor]) -> torch.Tensor:
        ...
        # [total_paths; embedding size]
        encoded_paths = self.node_embedding(contexts[PATH_NODES]).sum(0)

        # [total_paths; output size]
        output = self._concat_with_linear(
            [encoded_from_tokens, encoded_paths, encoded_to_tokens])
        return output
