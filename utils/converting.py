...
def tokens_to_wrapped_numpy(tokens: List[str],
                            to_id: Dict[str, int],
                            max_length: int,
    ...
    start_index = 0
    if is_wrapped:
        wrapped_numpy[0, 0] = sos_token
        start_index = 1

    for i, value in enumerate(tokens[:size]):
        wrapped_numpy[start_index + i, 0] = to_id.get(value, unk_token)

    if is_wrapped and len(tokens) < max_length:
        wrapped_numpy[start_index + len(tokens), 0] = eos_token
...
def strings_to_wrapped_numpy(
    ...
    for i, value in enumerate(values):
        ...
        wrapped_numpy[start_index:start_index + length, i] = [
            to_id.get(_t, unk_token) for _t in tokens[:length]
        ]
