...
def astminer_to_c2v(config: DictConfig):
    ...
    output_c2v_path = path.join(out_root, "all.c2v")
    if (path.exists(output_c2v_path)):
        print("visited!")
        return
    output_train_c2v_path = path.join(out_root, "train.c2v")
    output_test_c2v_path = path.join(out_root, "test.c2v")
    output_val_c2v_path = path.join(out_root, "val.c2v")
    output_lines = []
    labels = []
    out_path_contexts = []
    ...
    output_lines = []
    for md5 in tqdm(md5_to_lines, total=len(md5_to_lines)):
        if md5_to_lines[md5][0] != -1:
            ...
            cur_tokens = []
            cur_nodes = []
            for path_context in tqdm(path_contexts, total=len(path_contexts)):
    ...
    print("done!")
    with open(output_train_c2v_path, "a+") as c2v_output:
        c2v_output.write("".join(X_train))
    with open(output_test_c2v_path, "a+") as c2v_output:
        c2v_output.write("".join(X_test))
    with open(output_val_c2v_path, "a+") as c2v_output:
        c2v_output.write("".join(X_val))
